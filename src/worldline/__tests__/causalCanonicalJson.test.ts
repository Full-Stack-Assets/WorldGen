...
describe('canonical JSON', () => {
  ...
  it('normalizes Unicode and rejects ambiguous object shapes', () => {
    expect(canonicalize({ label: 'Cafe\u0301' })).toBe(canonicalize({ label: 'Café' }));
    expect(() => canonicalize({ e\u0301: 1, é: 2 })).toThrow('Canonical key collision');

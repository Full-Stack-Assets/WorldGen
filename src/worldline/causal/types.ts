...
export interface MechanismSpec {
  ...
  uncertainty: 'LOW' | 'MEDIUM' | 'HIGH' | 'UNQUANTIFIED';
  supportedRegimes: readonly string[];
  expectedEffects: readonly ExpectedDelta[];
  evidenceRefs: readonly string[];
  failureSemantics: 'REJECT' | 'UNKNOWN' | 'UNIDENTIFIED';
  authorityCeiling: 'AUTOMATIC_INTERNAL' | 'HUMAN_REQUIRED';
}

export interface ExpectedDelta {
  path: string;
  direction: 'INCREASE' | 'DECREASE' | 'SET' | 'UNCHANGED' | 'UNKNOWN';
  unit?: string;
  rationale: string;
}

export interface WorldTransitionProposal {
  schema: 'worldline-world-transition-proposal-v1';
  ...
  uncertainty: 'LOW' | 'MEDIUM' | 'HIGH' | 'UNQUANTIFIED';
  evidenceRefs: readonly string[];
  requestedAuthority: 'SANDBOX_ONLY' | 'CANONICAL_ADMISSION_REVIEW';

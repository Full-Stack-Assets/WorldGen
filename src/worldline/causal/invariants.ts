...
async function evaluateOne(ref: string, context: InvariantContext): Promise<InvariantResult> {
  ...
  if (ref === 'worldline-branch-rules-v1') {
    ...
    try {
      const passed = await branchCandidateMatchesRules(
...
export async function evaluateInvariantSuite(refs: readonly string[], context: InvariantContext): Promise<InvariantResult[]> {
  return Promise.all(refs.map((ref) => evaluateOne(ref, context)));
}

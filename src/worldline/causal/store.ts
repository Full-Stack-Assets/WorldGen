...
export interface CanonicalStoreSnapshot {
  revisions: Record<string, CanonicalRevision>;
  states: Record<string, unknown>;
  branchHeads: Record<string, string>;
  mechanisms: Record<string, TransitionMechanismArtifact>;
  receipts: Record<string, TransitionReceiptEnvelope>;
}
...
export function createInMemoryCanonicalStore(options: CanonicalStoreOptions = {}) {
  ...
  const exportSnapshot = (): CanonicalStoreSnapshot => ({
    revisions: sortedRecord(revisions.entries()),
    states: sortedRecord(states.entries()),
    branchHeads: sortedRecord(branchHeads.entries()),
    mechanisms: sortedRecord(mechanisms.entries()),
    receipts: sortedRecord(receipts.entries()),
  });

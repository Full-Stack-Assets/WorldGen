...
export function sha256Hex(input: string): string {
  ...
  for (let index = 0; index < 8; index += 1) {
    padded[totalLength - 1 - index] = Number(bitLength & 0xffn);
    bitLength >>= 8n;
  }
...
export function sha256Digest(input: string): Sha256Digest {
  return `sha256:${sha256Hex(input)}`;
}

export function hashCanonical(value: unknown): Sha256Digest {
  return sha256Digest(canonicalizeToJson(value));
}

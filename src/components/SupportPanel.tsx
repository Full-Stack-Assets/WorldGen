...
export function SupportPanel() {
  if (config.supportLinks.length === 0 && !config.proProductUrl) return null;

  return (
    <div className="panel-section support-panel">
      <h3>Support WorldGen</h3>
      {config.proProductUrl && (
        <a
          className="btn btn-accent btn-full support-pro-btn"
          href={config.proProductUrl}
          target="_blank"

...
export function ChronosExportPanel({ state }: { state: WorldlineState }) {
  const download = () => {
    ...
    anchor.href = url;
    anchor.download = 'worldline-chronos-v0.2.json';
    anchor.click();
  ...
  return (
    <section className="wl-chronos-export">
      ...
      <p>Exports committed world identity, time, branch ancestry, events, evidence labels, deterministic seeds, and replay commitments without binding to the active map or tile provider.</p>
      <button className="wl-primary" type="button" onClick={download}>Export Chronos v0.2 bundle</button>

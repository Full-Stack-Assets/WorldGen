...
export function useForgeController(
  ...
  const direct = useCallback(() => {
    const map = mapRef.current;
    if (!map) {
      setState((current) => ({ ...current, status: 'The Earth surface is still loading.' }));
      return;
    }
    ...
    void (async () => {
      for (const shot of variant.directorPath) {
        ...
        setState((current) => ({ ...current, status: `Director · ${shot.label}` }));
  ...
  const exportStill = useCallback(() => {
    const map = mapRef.current;
    if (!map) return;
    setState((current) => ({ ...current, status: 'Capturing the current FORGE frame.' }));
    void downloadForgeStill(map.getCanvas())
      .then(() => setState((current) => ({ ...current, status: 'PNG still exported.' })))
      .catch((error: unknown) => {
        setState((current) => ({
          ...current,
  ...
  const exportScene = useCallback(() => {
    ...
    try {
      ...
      setState((current) => ({ ...current, status: 'FORGE scene package exported.' }));
    } catch (error) {
      setState((current) => ({
        ...current,

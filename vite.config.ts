...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        // Precache the app shell so the whole experience works offline (the
        // scene is fully self-contained — no runtime asset fetches).
        globPatterns: ['**/*.{js,css,html,svg,png,ico,woff2}'],
        maximumFileSizeToCacheInBytes: 4 * 1024 * 1024,
        // Control the page on first activation so offline works on first visit;
        // skipWaiting stays off so updates surface via the in-app prompt.
        clientsClaim: true,
        skipWaiting: false,
        runtimeCaching: [
          {
